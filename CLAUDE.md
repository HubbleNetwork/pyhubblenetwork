...
- **`cli.py`** - Click-based CLI. Command groups: `ble` (scan, detect, check-time, validate), `ready` (scan, info, read-status, read-key-info, read-config, read-time, write-key, write-config, write-time, provision), `org` (info, list-devices, get-packets, register-device, delete-device, set-device-name), `sat` (scan, mock-scan; `scan` accepts `--key`/`--days`/`--counter-mode`/`--show-failed-decryption` to decrypt payloads locally — when `--key` is given without `--counter-mode`, the counter source is auto-detected (UNIX_TIME vs DEVICE_UPTIME) and announced, mirroring `ble scan`. `--key` also accepts several comma-separated keys: each packet is tried against every key in order, the first that decrypts wins, the counter source is auto-detected per key, and a `KEY` column — shown only when 2+ keys are given — displays the matching key's short prefix in the format the user supplied (`key` field in JSON output carries the full key token). A `DAY_OFFSET` column — shown whenever decrypting (any key) — gives the signed days forward/back used to decode each UNIX_TIME packet (0 = packet's own day), and `n/a` for DEVICE_UPTIME / undecrypted rows (`day_offset` field in JSON, omitted when n/a)). Top-level: `validate-credentials`.
