...
- **`ble.py`** - BLE scanning for beacon packets (UUID 0xFCA6). Uses `bleak` library. **One scanner, one event loop, per scan.** `scan_stream()` / `scan_stream_async()` are the primitives: they hold a single `BleakScanner` open and yield packets off an unbounded `asyncio.Queue` as they arrive, with `timeout` meaning the whole window rather than a per-packet wait. `scan()`, `scan_async()`, `scan_single()` and `scan_single_async()` are all thin wrappers over them. This shape is load-bearing, not stylistic: the old code built a fresh scanner inside a fresh `asyncio.run()` per call and the CLI looped it once per packet, which crashed on Linux (each iteration is a full D-Bus round trip to `bluetoothd`, BlueZ serialises discovery state machine-wide, and bleak force-finalises the dbus-fast bus of any closed loop) and silently dropped every advert that landed between packets on every platform. `tests/test_ble_stream.py` counts scanner lifecycles at both the SDK and CLI level so the churn can't come back. The sync entry points reject a running event loop **up front** via `_reject_running_loop()` rather than inferring it from a `RuntimeError` escaping `asyncio.run()`; the old handler couldn't tell a Jupyter problem from a BlueZ one and silently re-ran the whole scan on a new loop. `_shutdown_loop()` cancels leftovers the way `asyncio.run()` does, which is what keeps "Task was destroyed but it is pending" off the way out. `bleak>=0.22` is a floor, not a preference: earlier versions share one BlueZ manager across event loops. Automatically detects encrypted vs unencrypted protocol packets. Unencrypted protocol (version 1) packets carry a 34-bit network ID and up to 18 bytes of customer payload; `parse_unencrypted()` extracts these fields. AES-EAX protocol (version 2) packets carry a 2-byte nonce salt, 8-byte EID, encrypted payload (0-9 bytes), and 4-byte auth tag; `AesEaxPacket` stores these parsed fields. Unknown protocol versions (3+) are captured as `UnknownPacket`.
...
- **`termcaps.py`** - The only module in the package allowed to contain non-ASCII, enforced by `tests/test_terminal_compat.py::TestSourceStaysAscii` (ruff can't do this: RUF001-003 only flag *confusable* characters, so box-drawing sails through). Holds two `Glyphs` instances, `UNICODE` and `ASCII`, whose every field must have identical `len()` so the fixed-width tables keep their arithmetic. `ascii_mode()` resolves `--ascii/--no-ascii` > `HUBBLE_ASCII` > an encoding probe derived from the glyph set itself (not a codec denylist, which is why `latin-1` is correctly rejected: it encodes `·` but not `─`). `color_enabled()`/`force_color()` implement `NO_COLOR`/`FORCE_COLOR`, which **Click does not support at any version**. Colour is applied in the *root group callback* because Click copies `color` from parent to child when a context is constructed, so setting it later reaches nothing.

- **`doctor`** - `hubblenetwork doctor` is the setup validation step: credentials (present, and valid via `cloud.get_env_from_credentials`), Bluetooth, Docker, and the cached receiver image. Exits 1 if any check FAILS; SKIP never fails a run. The Bluetooth check is deliberately **static** on both platforms, because the point is to report the problem rather than reproduce it. On macOS `_bluetooth_usage_description()` reads the interpreter's `Info.plist` looking for `NSBluetoothAlwaysUsageDescription`, because touching CoreBluetooth without that key is what aborts the process. On Linux `_bluetooth_linux()` reads sysfs, `/run` and the group database in fix-this-first order: adapter present in `/sys/class/bluetooth`, not rfkill-blocked, a system D-Bus socket to reach `bluetoothd` on, then `bluetooth` group membership. A **missing** `bluetooth` group is not a failure (Arch and Fedora ship a polkit rule and no such group; only Debian and Ubuntu gate the D-Bus policy on it), and root is never told to join a group. The three paths are module constants (`_SYSFS_BLUETOOTH`, `_SYSFS_RFKILL`, `_DBUS_SYSTEM_SOCKET`) so tests can point them at a fixture tree. Linux used to fall through to SKIP, which meant `doctor` had nothing to say to the one platform where BLE setup actually goes wrong. Tests in `tests/test_cli_doctor.py`.

- **Help and error-recovery contract** - Every group uses `cls=HubbleGroup`, which (a) renders help as a table of real invocations built from the Click tree via `_walk_commands()` / `_invocation()`, so a new command cannot go undocumented, and (b) overrides `resolve_command` to search the *whole* tree on an unknown name — Click only checks siblings, so from the root it can't see that `list-devices` lives under `org`. Ordering and blurbs come from `_GROUP_ORDER` / `_GROUP_BLURB`; the four examples come from `_START_HERE`; `_HELP_COL = 35` keeps the widest row inside 80. Every command carries a plain-language `short_help=` (asserted present and ≤44 chars for all 26). Required arguments use `cls=GuidedArgument` with a `metavar` (`<id>`, `<seconds>`) and `guidance` naming how to find the value. `_credentials_error()` distinguishes not-set from rejected; `_get_env_or_fail()` names the export. `main()` handles `UsageError` separately so the usage preamble survives, a message that is already a usage block gets no `Error:` prefix, and an unrecognised option gets the command's accepted option list appended. `validate-credentials` exits 1 on failure (it used to exit 0, making failure undetectable in scripts). Tests in `tests/test_cli_help_and_errors.py` lock all of this, including `TestReadmeStaysTrue`, which resolves every `hubblenetwork ...` line in `README.md` **and `plugins/**/*.md`** through the real Click tree (66 invocations today). It reads only shell-tagged fences, so illustrative error output in a bare fence is not treated as a runnable example. The plugin skill docs had drifted to documenting `--pool-size` and `ready provision --format json`, neither of which exist, precisely because nothing checked them.
...
- **Sync/Async duality**: BLE and provisioning functions have both sync and async variants. The BLE sync variants run one event loop for the whole scan and reject a running loop up front (see `ble.py`); they do **not** infer the Jupyter case from a `RuntimeError`, because BlueZ raises those too. `ready.py` still uses `asyncio.run()` with fallback handling for existing event loops.
