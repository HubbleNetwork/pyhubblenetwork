...
- **`org.py`** - `Organization` class: credential-scoped operations (register devices, retrieve packets, list devices). Automatically resolves environment (PROD/TESTING) from credentials. `get_device(device_id)` fetches a single device by ID, returning `None` when not found.
...
- **`detect.py`** - Decryption-strategy auto-detection, decoupled from the CLI (imports no `click`, never prints). `detect_key_config(key, pkts)` detects the full config — encryption variant, counter source, and EAX period exponent — from a key and scanned packets, returning `(LocalKeyConfig, packet, decrypted)` or `(None, None, None)`. `compare_key_config(local, *, encryption, counter_source, period_exponent)` diffs a detected `LocalKeyConfig` against backend-registered values, returning per-field `FieldResult`s (status: `ok` / `fail` / `skipped`). `CtrCounterModeDetector` and `EaxExponentDetector` are per-scan stateful objects that hold the detection cache and own the detect/sweep loop; their `decrypt()` takes a caller-supplied packet-bound `decrypt_fn` and returns a `Detection(result, label)` — `label` is set only on the first successful detection of a scan. The CLI builds the `decrypt_fn` (so test mocks of `cli.decrypt`/`cli.decrypt_eax` still apply) and prints the `[INFO] Detected:` line itself when `label` is set.
...
- **`cli.py`** - Click-based CLI. Command groups: `ble` (scan, detect, check-time, validate), `ready` (scan, info, read-status, read-key-info, read-config, read-time, write-key, write-config, write-time, provision), `org` (info, list-devices, get-packets, register-device, delete-device, set-device-name), `sat` (scan, mock-scan; `scan` accepts `--key`/`--days`/`--counter-mode`/`--show-failed-decryption` to decrypt payloads locally — when `--key` is given without `--counter-mode`, the counter source is auto-detected (UNIX_TIME vs DEVICE_UPTIME) and announced, mirroring `ble scan`). Top-level: `validate-credentials`. `ble validate` fetches the device via `get_device`, then after decrypting compares the detected key config (encryption / counter_source / period_exponent) against the backend registration via `compare_key_config`, hard-failing on any `fail` result and skipping the comparison when the backend omits config fields; EAX devices validate config then skip the CTR-only ingest/retrieve steps.
